...
opt = input_data.read_data_sets("OPT_data/", one_hot=True)
sess = tf.Session()
...
trainwriter=tf.summary.FileWriter('data/opt_model'+'/logs/train',sess.graph)

init = tf.global_variables_initializer()
sess.run(init)

for i in range(1000):
    batch_xs, batch_ys = opt.train.next_batch(100)
    summary, _ = sess.run([merged, train_step], feed_dict={x: batch_xs, y_: batch_ys})
    trainwriter.add_summary(summary, i)

# model export path
export_path = 'data/opt_model'
...
saver.save(sess, '/home/manikanta/tensorflow/opt_model')
"""